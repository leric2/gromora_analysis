...
#locate the latitude close to bern
for lat_num, lat_GDOC in enumerate(GDOC_lat):
    if lat_GDOC == 47.5:
        lat_certain = lat_num

#pick up 8 am data
for hour_num, GDOC_hour in enumerate(GDOC_hour):
    if GDOC_hour == 8.0:
        hour_certain = hour_num
...
month_title_12 = (
'1718av_gromos-January-full-pressure-ozone-diurnal-ts', '1718av_gromos-February-full-pressure-ozone-diurnal-ts',
'1718av_gromos-March-full-pressure-ozone-diurnal-ts'
, '1718av_gromos-April-full-pressure-ozone-diurnal-ts', '1718av_gromos-May-full-pressure-ozone-diurnal-ts',
'1718av_gromos-June-full-pressure-ozone-diurnal-ts'
, '1718av_gromos-July-full-pressure-ozone-diurnal-ts', '1718av_gromos-August-full-pressure-ozone-diurnal-ts',
'1718av_gromos-September-full-pressure-ozone-diurnal-ts'
, '1718av_gromos-October-full-pressure-ozone-diurnal-ts', '1718av_gromos-November-full-pressure-ozone-diurnal-ts',
'1718av_gromos-December-full-pressure-ozone-diurnal-ts')

month_title_somora_12 = (
'1718av_somora-January-full-pressure-ozone-diurnal-ts', '1718av_somora-February-full-pressure-ozone-diurnal-ts',
'1718av_somora-March-full-pressure-ozone-diurnal-ts'
, '1718av_somora-April-full-pressure-ozone-diurnal-ts', '1718av_somora-May-full-pressure-ozone-diurnal-ts',
'1718av_somora-June-full-pressure-ozone-diurnal-ts'
, '1718av_somora-July-full-pressure-ozone-diurnal-ts', '1718av_somora-August-full-pressure-ozone-diurnal-ts',
'1718av_somora-September-full-pressure-ozone-diurnal-ts'
, '1718av_somora-October-full-pressure-ozone-diurnal-ts', '1718av_somora-November-full-pressure-ozone-diurnal-ts',
'1718av_somora-December-full-pressure-ozone-diurnal-ts')
...
'comparison-January-full-pressure-ozone-variation-4pm', 'comparison-February-full-pressure-ozone-variation-4pm',
'comparison-March-full-pressure-ozone-variation-4pm'
, 'comparison-April-full-pressure-ozone-variation-4pm', 'comparison-May-full-pressure-ozone-variation-4pm',
'comparison-June-full-pressure-ozone-variation-4pm'
, 'comparison-July-full-pressure-ozone-variation-4pm', 'comparison-August-full-pressure-ozone-variation-4pm',
'comparison-September-full-pressure-ozone-variation-4pm'
, 'comparison-October-full-pressure-ozone-variation-4pm', 'comparison-November-full-pressure-ozone-variation-4pm',
'comparison-December-full-pressure-ozone-variation-4pm')
...
for month_data, month_title in enumerate(np.arange(12)):
    ...
    gromos_month_data_std_full = np.zeros((1, 47))
    gromos_month_data_capacity_ch_full = np.array([])
    for x in range(0, 24):
        time_month_data = np.array([])
        gromos_month_data_std_ch = np.array([])
        ...
        for num_std in range(len(gromos.o3_p)):    #caculate std at each pressure lev
            gromos_month_data_std = np.std(gromos_month_data[:,num_std])
            gromos_month_data_std_ch = np.append(gromos_month_data_std_ch, gromos_month_data_std)
        gromos_month_data_capacity_ch = gromos_month_data.size/47 #size of data of each hour
        gromos_month_data_capacity_ch_full = np.append(gromos_month_data_capacity_ch_full, gromos_month_data_capacity_ch)
        gromos_month_data_std_full = np.vstack((gromos_month_data_std_full, gromos_month_data_std_ch))
    ...
    gromos_month_data_std_full = np.delete(gromos_month_data_std_full, 0, axis=0)
    gromos_month_data_std_0 = gromos_month_data_std_full[0]
    gromos_month_data_std_7 = gromos_month_data_std_full[7]
    average_midnight = (gromos_month_data_av_full[0] + gromos_month_data_av_full[
        23]) / 2  # average 0am and 23pm as midnight reference
    gromos_month_data_av_full_rate_midnight = (gromos_month_data_av_full/ average_midnight)  # get a rate arrary (every row divide into midnight row)
    gromos_month_data_av_full_rate_midnight_1 = np.transpose(gromos_month_data_av_full_rate_midnight[7]) #pick up 8 am ozone array
    sigma_f = (gromos_month_data_av_full_rate_midnight[7]) * np.sqrt(        #uncertainly propagation to calculate std of the ratio variable
        np.square(gromos_month_data_std_7 / gromos_month_data_av_full[7])
        + np.square(gromos_month_data_std_0 / average_midnight))
    sigma_f = np.transpose(sigma_f)
    SEM_f = sigma_f / np.sqrt(gromos_month_data_capacity_ch_full[7]) #calculate SEM

    gromos_month_1_data_av_full = np.zeros((1, 47))  # build a one row full 0 array
    gromos_month_data_std_full_1 = np.zeros((1, 47))
    gromos_month_data_capacity_ch_full_1 = np.array([])
    for x in range(0, 24):
        time_month_data = np.array([])
        gromos_month_data_std_ch_1 = np.array([])
        ...
        for num_std in range(len(gromos.o3_p)):
            gromos_month_data_std_1 = np.std(gromos_month_1_data[:,num_std]) #get std for each pressure (1 certain hour)
            gromos_month_data_std_ch_1 = np.append(gromos_month_data_std_ch_1, gromos_month_data_std_1)
        gromos_month_data_capacity_ch_1 = gromos_month_data.size/47 #size of data of each hour in one month
        gromos_month_data_capacity_ch_full_1 = np.append(gromos_month_data_capacity_ch_full_1, gromos_month_data_capacity_ch_1)
        gromos_month_data_std_full_1 = np.vstack((gromos_month_data_std_full_1, gromos_month_data_std_ch_1))
    ...
    gromos_month_data_std_full_1 = np.delete(gromos_month_data_std_full, 0, axis=0)
    gromos_month_1_data_std_0 = gromos_month_data_std_full_1[0]# 0 oclock std for whole pressure
    gromos_month_1_data_std_7 = gromos_month_data_std_full_1[7]#17 oclock std for whole pressure
    average_midnight_1 = (gromos_month_1_data_av_full[0] + gromos_month_1_data_av_full[23]) / 2  # average 0 and 23 as midnight reference
    gromos_month_1_data_av_full_rate_midnight = (gromos_month_1_data_av_full / average_midnight_1)  # get a rate arrary (every row divide into midnight row)
    gromos_month_data_av_full_rate_midnight_2 = np.transpose(gromos_month_1_data_av_full_rate_midnight[7])
    ...
    sigma_f_1 = (gromos_month_1_data_av_full_rate_midnight[7]) * np.sqrt(
        np.square(gromos_month_1_data_std_7 / gromos_month_1_data_av_full[7])
        + np.square(gromos_month_1_data_std_0 / average_midnight_1)) #uncertainly propogation formula
    sigma_f_1 = np.transpose(sigma_f_1)
    SEM_f_1 = sigma_f_1 / np.sqrt(gromos_month_data_capacity_ch_full_1[7]) # mean error (SEM)

    SEM_f_t = (SEM_f + SEM_f_1)/2 #average the two year SEM

    ####################################################################somora
    somora_month_data_av_full = np.zeros((1, 47))  # build a one row full 0 array
    somora_month_data_std_full = np.zeros((1, 47))
    somora_month_data_capacity_ch_full = np.array([])
    for x in range(0, 24):
        time_month_data = np.array([])
        somora_month_data_std_ch = np.array([])
        ...
        for num_std in range(len(somora.o3_p)):
            somora_month_data_std = np.std(somora_month_data[:,num_std])
            somora_month_data_std_ch = np.append(somora_month_data_std_ch, somora_month_data_std)
        somora_month_data_capacity_ch = somora_month_data.size/47 #size of data of each hour
        somora_month_data_capacity_ch_full = np.append(somora_month_data_capacity_ch_full, somora_month_data_capacity_ch)
        somora_month_data_std_full = np.vstack((somora_month_data_std_full, somora_month_data_std_ch))
    ...
    somora_month_data_std_full = np.delete(somora_month_data_std_full, 0, axis=0)
    somora_month_data_std_0 = somora_month_data_std_full[0]
    somora_month_data_std_7 = somora_month_data_std_full[7]
    #somora_month_data_std_7 = somora_month_data_std_full[7]
    average_midnight_2 = (somora_month_data_av_full[0] + somora_month_data_av_full[
        23]) / 2  # average 0am and 23pm as midnight reference
    somora_month_data_av_full_rate_midnight = somora_month_data_av_full / average_midnight_2  # get a rate arrary (every row divide into midnight row)
    somora_month_data_av_full_rate_midnight_1 = np.transpose(somora_month_data_av_full_rate_midnight[7])
    #somora_month_data_av_full_rate_midnight_1 = np.transpose(somora_month_data_av_full_rate_midnight[7])
    sigma_f_2 = (somora_month_data_av_full_rate_midnight[7]) * np.sqrt(
        np.square(somora_month_data_std_7 / somora_month_data_av_full[7])
        + np.square(somora_month_data_std_0 / average_midnight_2))
    '''
    sigma_f_2 = (somora_month_data_av_full_rate_midnight[7]) * np.sqrt(
       np.square(somora_month_data_std_7 / somora_month_data_av_full[7])
        + np.square(somora_month_data_std_0 / average_midnight_2))
    '''
    sigma_f_2 = np.transpose(sigma_f_2)
    SEM_f_2 = sigma_f_2 / np.sqrt(somora_month_data_capacity_ch_full[7])
    #SEM_f_2 = sigma_f_2 / np.sqrt(somora_month_data_capacity_ch_full[7])

    somora_month_1_data_av_full = np.zeros((1, 47))  # build a one row full 0 array
    somora_month_data_std_full_1 = np.zeros((1, 47))
    somora_month_data_capacity_ch_full_1 = np.array([])
    for x in range(0, 24):
        time_month_data = np.array([])
        somora_month_data_std_ch_1 = np.array([])
        ...
        for num_std in range(len(somora.o3_p)):
            somora_month_data_std_1 = np.std(somora_month_1_data[:,num_std])
            somora_month_data_std_ch_1 = np.append(somora_month_data_std_ch_1, somora_month_data_std_1)
        somora_month_data_capacity_ch_1 = somora_month_data.size/47 #size of data of each hour
        somora_month_data_capacity_ch_full_1 = np.append(somora_month_data_capacity_ch_full_1, somora_month_data_capacity_ch_1)
        somora_month_data_std_full_1 = np.vstack((somora_month_data_std_full_1, somora_month_data_std_ch_1))
    ...
    somora_month_data_std_full_1 = np.delete(somora_month_data_std_full, 0, axis=0)
    somora_month_1_data_std_0 = somora_month_data_std_full_1[0]
    somora_month_1_data_std_7 = somora_month_data_std_full_1[7]
    #somora_month_1_data_std_7 = somora_month_data_std_full_1[7]
    average_midnight_3 = (somora_month_1_data_av_full[0] + somora_month_1_data_av_full[
        23]) / 2  # average 0am and 23pm as midnight reference
    somora_month_1_data_av_full_rate_midnight = somora_month_1_data_av_full / average_midnight_3  # get a rate arrary (every row divide into midnight row)
    somora_month_data_av_full_rate_midnight_2 = np.transpose(somora_month_1_data_av_full_rate_midnight[7])
    #somora_month_data_av_full_rate_midnight_2 = np.transpose(somora_month_1_data_av_full_rate_midnight[7])
    ...
    sigma_f_3 = (somora_month_1_data_av_full_rate_midnight[7]) * np.sqrt(
        np.square(somora_month_1_data_std_7 / somora_month_1_data_av_full[7])
        + np.square(somora_month_1_data_std_0 / average_midnight_3))
    sigma_f_3 = np.transpose(sigma_f_3)
    SEM_f_3 = sigma_f_3 / np.sqrt(somora_month_data_capacity_ch_full_1[7])

    SEM_f_t_1 = (SEM_f_2 + SEM_f_3)/2


    ###################################################GDOC
    GDOC_month = GDOC_month_12_data[month_data]
    GDOC_month = GDOC_month.transpose()
    GDOC_month_ct = np.array(GDOC_month[:,hour_certain])
    #####################################################WACCUM
    ...
    WACCM_month_data_av_full_rate_midnight = np.transpose(
        WACCM_month_data_av_full[7] / average_midnight)  # get a rate arrary (every row divide into midnight row)
...
############################################################################ plot pressure to
    ...
    #ax.set_xlim(0.1,2)
    ax.set_xlabel('Ratio to ozone at midnight')
    ax.set_ylabel('Pressure [hPa]')
    ax.set_title('Ozone variation to pressure comparison at 8am'+ months_12_title[month_data])
    ax.set_yscale('log')
    ax.plot(gromos_month_data_av_full_rate_midnight_12, gromos.o3_p/100, color='#8dd3c7', label='gromos_av')
    ax.fill_betweenx(gromos.o3_p/100, gromos_month_data_av_full_rate_midnight_12+SEM_f_t,
                    gromos_month_data_av_full_rate_midnight_12-SEM_f_t, facecolor='#b2e2e2', alpha=0.4)
    #ax.errorbar(gromos_month_data_av_full_rate_midnight_12, gromos.o3_p/100, xerr=SEM_f_t,
    #           fmt='x', ms=4, color='#8dd3c7', capsize=3, capthick=1.5, label='gromos_av')
    ax.plot(somora_month_data_av_full_rate_midnight_12, somora.o3_p/100, color='orange', label='somora_av')
    ax.fill_betweenx(somora.o3_p/100, somora_month_data_av_full_rate_midnight_12 + SEM_f_t_1,
                     somora_month_data_av_full_rate_midnight_12 - SEM_f_t_1, facecolor='#fdbe85', alpha=0.4)
    #ax.errorbar(somora_month_data_av_full_rate_midnight_12, somora.o3_p/100, xerr=SEM_f_t_1,
    #            fmt='o', ms=4, color='orange', capsize=3, capthick=1.5, label='somora_av')
    plt.gca().invert_yaxis()
    ax.plot(GDOC_month_ct, GDOC_pressure, color='#984ea3', label='GDOC', linestyle='-.')
    ax.plot(WACCM_month_data_av_full_rate_midnight, WACCM_pressure, color='#e41a1c', label='WACCM', linestyle=':')
    plt.grid(which='both')
    plt.legend(loc='best')
    #plt.savefig('C:/Users/Hou/Desktop/master thesis/figures for 2018 data/2021.11/1718av_pressure to ozone at 4pm/' + comparison_month_title_12[month_title] + '.pdf', dpi=400)
    plt.savefig('C:/Users/Hou/Desktop/master thesis/figures for 2018 data/2021.11/1718av_pressure to ozone at 8am/' + comparison_month_title_12[month_title] + '.pdf', dpi=400)
    plt.show()
...
'''
    ...
    plt.savefig('C:/Users/Hou/Desktop/master thesis/figures for 2018 data/2021.8/comparison full pressure/' +
                comparison_month_title_12[month_title] + '.png', dpi=100)
    plt.show()
