...
filename = 'F:\PyCharm Community Edition 2019.3\practice2021 april/old_ret.nc'
gromos_2 = xr.open_dataset(
    filename,
    decode_times=True,
    decode_coords=True,
    # use_cftime=True,
...
gromos_2_o3 = (gromos_2.o3)*1e6
gromos_2_p = gromos_2.p
gromos_2_time = gromos_2.time.values
'''
py_datetime = np.array([])
#datetime_trans = datetime.fromordinal(int(gromos_2_time[0])) + timedelta(days=gromos_2_time[0]%1) - timedelta(days = 366)
for datetime_num, datetime in enumerate(gromos_2_time):
    datetime_trans = datetime.fromordinal(int(datetime)) + timedelta(days=datetime%1) - timedelta(days = 366)
    py_datetime = np.append(py_datetime, datetime_trans)
'''
datetime_py = pd.to_datetime(gromos_2_time-719529, unit='D')

gromos_2 = xr.Dataset(
    {'o3_x':(['p','time'], gromos_2_o3)},
    coords=dict({
            'o3_p': gromos_2_p,
            'time': datetime_py,
        }),
)
gromos_2 = gromos_2.sel(time=slice("2017-01-01", "2017-12-31"))
...
gromos_month_1 = gromos_2.o3_x.sel(time=slice("2017-01-01", "2017-01-31"))
gromos_month_2 = gromos_2.o3_x.sel(time=slice("2017-02-01", "2017-02-28"))  # 2017 slice month o3 data
gromos_month_3 = gromos_2.o3_x.sel(time=slice("2017-03-01", "2017-03-31"))
gromos_month_4 = gromos_2.o3_x.sel(time=slice("2017-04-01", "2017-04-30"))
gromos_month_5 = gromos_2.o3_x.sel(time=slice("2017-05-01", "2017-05-31"))
gromos_month_6 = gromos_2.o3_x.sel(time=slice("2017-06-01", "2017-06-30"))
gromos_month_7 = gromos_2.o3_x.sel(time=slice("2017-07-01", "2017-07-31"))
gromos_month_8 = gromos_2.o3_x.sel(time=slice("2017-08-01", "2017-08-31"))
gromos_month_9 = gromos_2.o3_x.sel(time=slice("2017-09-01", "2017-09-30"))
gromos_month_10 = gromos_2.o3_x.sel(time=slice("2017-10-01", "2017-10-31"))
gromos_month_11 = gromos_2.o3_x.sel(time=slice("2017-11-01", "2017-11-30"))
gromos_month_12 = gromos_2.o3_x.sel(time=slice("2017-12-01", "2017-12-31"))

gromos_month_lst_1 = gromos_2.sel(time=slice("2017-01-01", "2017-01-31"))
gromos_month_lst_2 = gromos_2.sel(time=slice("2017-02-01", "2017-02-28"))  # 2017 slice month data
gromos_month_lst_3 = gromos_2.sel(time=slice("2017-03-01", "2017-03-31"))
gromos_month_lst_4 = gromos_2.sel(time=slice("2017-04-01", "2017-04-30"))
gromos_month_lst_5 = gromos_2.sel(time=slice("2017-05-01", "2017-05-31"))
gromos_month_lst_6 = gromos_2.sel(time=slice("2017-06-01", "2017-06-30"))
gromos_month_lst_7 = gromos_2.sel(time=slice("2017-07-01", "2017-07-31"))
gromos_month_lst_8 = gromos_2.sel(time=slice("2017-08-01", "2017-08-31"))
gromos_month_lst_9 = gromos_2.sel(time=slice("2017-09-01", "2017-09-30"))
gromos_month_lst_10 = gromos_2.sel(time=slice("2017-10-01", "2017-10-31"))
gromos_month_lst_11 = gromos_2.sel(time=slice("2017-11-01", "2017-11-30"))
gromos_month_lst_12 = gromos_2.sel(time=slice("2017-12-01", "2017-12-31"))
...
month_title_12 = ('2017-January-stratospheric-ozone-diurnal-cycle[old retrieval]', '2017-February-stratospheric-ozone-diurnal-cycle[old retrieval]',
                  '2017-March-stratospheric-ozone-diurnal-cycle[old retrieval]'
                  , '2017-April-stratospheric-ozone-diurnal-cycle[old retrieval]', '2017-May-stratospheric-ozone-diurnal-cycle[old retrieval]',
                  '2017-June-stratospheric-ozone-diurnal-cycle[old retrieval]'
                  , '2017-July-stratospheric-ozone-diurnal-cycle[old retrieval]', '2017-August-stratospheric-ozone-diurnal-cycle[old retrieval]',
                  '2017-September-stratospheric-ozone-diurnal-cycle[old retrieval]'
                  , '2017-October-stratospheric-ozone-diurnal-cycle[old retrieval]', '2017-November-stratospheric-ozone-diurnal-cycle[old retrieval]',
                  '2017-December-stratospheric-ozone-diurnal-cycle[old retrieval]')
...
for month_data, month_title in enumerate(np.arange(12)):
    ...
    minute = pandas_time_test.minute
    ...
    for sunrise_dayofmonth in sunrise_set_day_order:
        ...
        surnise_set_lst = get_sunset_lst_from_lst(sunrise_set_lst_covert, 46.82)  # surise_set_lst_covert[0] is that pick time variable
    ...
    gromos_month_data_av_full = np.zeros((1, 51))  # build a one row full 0 array
    for x in range(0, 24):
        time_month_data = np.array([])
        time_month_data_int = np.array([])
        time_month_data_half = np.array([])
        ...
        minute_new = minute[time_month_data]
        for a, b in enumerate(minute_new):
            if  b <= 30 :
                time_month_data_int = np.append(np.array(time_month_data_int,dtype=int), a)
            else:
                time_month_data_half = np.append(np.array(time_month_data_half,dtype=int), a)
        gromos_month_data = gromos_month[:, time_month_data].values
        gromos_month_data_int= np.transpose(gromos_month_data[:, time_month_data_int]) # pick up o3_x data corresponding to index hour
        gromos_month_data_half = np.transpose(gromos_month_data[:, time_month_data_half])  # pick up o3_x data corresponding to index hour
        gromos_month_data_av_int = np.mean(gromos_month_data_int, axis=0)  # sum all the data along row
        gromos_month_data_av_half = np.mean(gromos_month_data_half, axis=0)  # sum all the data along row
        gromos_month_data_av_full = np.vstack((gromos_month_data_av_full, gromos_month_data_av_int))  # build a diuranl ts array
        gromos_month_data_av_full = np.vstack((gromos_month_data_av_full, gromos_month_data_av_half))  # build a diuranl ts array

    gromos_month_data_av_full = np.delete(gromos_month_data_av_full, 0, axis=0)  # delete the first row (full 0 array)
    average_midnight = (gromos_month_data_av_full[0] + gromos_month_data_av_full[47]) / 2  # average 0am and 23pm as midnight reference
    ...
    ax.set_ylim(1, 100)
    cs = plt.contourf(np.arange(0, 24, 0.5), gromos_2_p / 100, gromos_month_data_av_full_rate_midnight, levels=(interval),
    ...
    plt.savefig('C://Users//Hou//Desktop//master thesis//figures for 2018 data//2021.11//old retrieval//' +month_title_12[month_title] + '.pdf', dpi=100)
    plt.show()